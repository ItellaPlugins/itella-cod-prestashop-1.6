...
{capture name=path}
	<a href="{$link->getPageLink('order', true, NULL, "step=3")|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='Go back to the Checkout' mod='itellacod'}">{l s='Checkout' mod='itellacod'}</a><span class="navigation-pipe">{$navigationPipe}</span>{l s='Itella card on delivery (COD) payment' mod='itellacod'}
{/capture}
...
<h3>{l s='Itella card on delivery (COD) payment' mod='itellacod'}</h3>

<form action="{$link->getModuleLink('itellacod', 'validation', [], true)|escape:'html'}" method="post">
	<input type="hidden" name="confirm" value="1" />
	<p>
		<img src="{$this_path_cod}logo.png" alt="{l s='Itella card on delivery (COD) payment' mod='itellacod'}" 
		style="float:left; margin: 0px 10px 5px 0px; height: 33px;" />
		{l s='You have chosen Itella card on Delivery method.' mod='itellacod'}
		<br/><br />
